...
func TestDecodeAtom(t *testing.T) {
	expected := gen.Atom("abc")
	packet := []byte{ettAtomUTF8, 0, 3, 97, 98, 99}
	term, _, err := Decode(packet, []gen.Atom{}, DecodeOptions{})
...
func TestDecodeList(t *testing.T) {
	expected := List{3.14, gen.Atom("abc"), int64(987654321)}
...
func TestDecodeTuple(t *testing.T) {
	expected := Tuple{3.14, gen.Atom("abc"), int64(987654321)}
...
func TestDecodePidWithCacheAtom(t *testing.T) {
	...
	cache := []gen.Atom{gen.Atom("erl-demo@127.0.0.1")}
...
func TestDecodeComplex(t *testing.T) {
	...
	expected := Tuple{"hello", List{},
		Map{gen.Atom("v1"): List{Tuple{3, 13, 3.13}, Tuple{gen.Atom("abc"), "abc"}},
			gen.Atom("v2"): int64(12345)}}
...
func BenchmarkDecodeAtom(b *testing.B) {
	packet := []byte{ettAtomUTF8, 0, 3, 97, 98, 99}
	for i := 0; i < b.N; i++ {
		_, _, err := Decode(packet, []gen.Atom{}, DecodeOptions{})
		if err != nil {
			b.Fatal(err)
		}
	}
