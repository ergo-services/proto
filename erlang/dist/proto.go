...
func (dp *distProto) Init(ctx context.Context, conn lib.NetReadWriter, nodename string, details node.HandshakeDetails) (node.ConnectionInterface, error) {
	...
	linkQueue := newLinkWriterQueue(conn)
	for i := 0; i < numHandlers; i++ {
		// run writer routines (encoder)
		send := make(chan sendMessage, dp.options.SendQueueLength)
		keepAlive := i == 0
		writer := newLinkWriter(linkQueue, defaultLatency, keepAlive)
		connection.senders.sender[i] = &senderChannel{
			sendChannel: send,
			writer:      writer,
		}
		go connection.sender(i, send, writer, dp.options, connection.flags)
	}
	...
	case etf.Tuple:
		...
		case int:
			...
			case distProtoSEND:
				...
				// workaround to support reply on sync request (gen.ServerProcess.Call)
				switch m := message.payload.(type) {
				case etf.Tuple:
					if len(m) != 2 {
						break
					}
					switch ref := m.Element(1).(type) {
					case etf.Ref:
						dc.router.RouteReply(etf.Pid{}, t.Element(3).(etf.Pid), ref, m.Element(2))
						return nil
					}
				}
				//
...
func (dc *distConnection) sender(sender_id int, send <-chan sendMessage, writer *linkWriter, options node.ProtoOptions, peerFlags node.Flags) {
	...
	for {
		...
		if message.packet != nil {
			...
			bytesOut, err := writer.Write(message.packet.B)
			if err != nil {
				return
			}
		...
		for {
			...
			if !fragmentationEnabled || lenMessage < options.FragmentationUnit {
				...
				if message.proxy == nil {
					...
					bytesOut, err := writer.Write(packetBuffer.B[startDataPosition:])
					if err != nil {
						return
					}
				...
				bytesOut, err := writer.Write(xBuffer.B)
				if err != nil {
					return
				}
				atomic.AddUint64(&dc.stats.BytesOut, uint64(bytesOut))
			...
			if message.proxy == nil {
				binary.BigEndian.PutUint32(packetBuffer.B[startDataPosition:], uint32(lenPacket))
				packetBuffer.B[startDataPosition+4] = protoDist // 131
				bytesOut, err := writer.Write(packetBuffer.B[startDataPosition : startDataPosition+4+lenPacket])
				if err != nil {
					return
				}
				atomic.AddUint64(&dc.stats.BytesOut, uint64(bytesOut))
				...
				if encryptionEnabled == false {
					...
					bytesOut, err := writer.Write(packetBuffer.B[startDataPosition-32 : startDataPosition+4+lenPacket])
					if err != nil {
						return
					}
					atomic.AddUint64(&dc.stats.BytesOut, uint64(bytesOut))
